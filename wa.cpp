...
statWA wanalysis(const Task& ti, const std::vector<Task>& hps, const int m, const int K) {
	...
	try {
		...
		*/
		...
		// ***************** REFINED ANALYSIS ****************************** //
		// (4.4.1) "sjk" is the activation time of the last job of task j before f_k
		...
		for (unsigned int j = 1; j <= hps.size(); j++) {
			for (unsigned int k = 1; k <= K; k++) {
				// sjk definition
				model.add(IloRange(env, 0, sjk[j - 1][k - 1] - alpha[j - 1] - (nf[j - 1][k - 1] - 1)*p[j - 1] + jitt[j - 1], IloInfinity));
				model.add(IloRange(env, -IloInfinity, sjk[j - 1][k - 1] - alpha[j - 1] - (nf[j - 1][k - 1] - 1)*p[j - 1] - jitt[j - 1], 0));
				// sjk bounds
				model.add(IloRange(env, br[j-1], f[k - 1] - sjk[j - 1][k - 1], p[j - 1] + jitt[j - 1]));
			}
		}
			...
			for (unsigned int s = 1; s <= hps.size(); s++) {
				...
				for (unsigned int j = 1; j <= hps.size(); j++) {
					if (s == j) // if s==j then nsjk = nfjk - 1
						model.add(IloRange(env, 0, nsjk[s - 1][j - 1][k - 1] - (nf[j - 1][k - 1] - 1), 0));
					else {
						// upper bound
						model.add(IloRange(env, -IloInfinity, nsjk[s - 1][j - 1][k - 1] - nf[j - 1][k - 1], 0));

						//// nsjk 
						model.add(IloRange(env, 0, nsjk[s - 1][j - 1][k - 1] * p[j - 1] - (S - alpha[j - 1] - jitt[j - 1]), IloInfinity));
						model.add(IloRange(env, -IloInfinity, (nsjk[s - 1][j - 1][k - 1] - 1) * p[j - 1] + sigma1 - (S - alpha[j - 1] + jitt[j - 1]), 0));
					}
				}
			}
		}
			...
			for (unsigned int j = 1; j <= hps.size(); j++) {
				...
				for (unsigned int k = 1; k <= K; k++) {
					...
					for (unsigned int i = 1; i <= ceil(ubs / p[j - 1]); i++) {
						string name = "nsb_" + convert_to_string(s) + convert_to_string(j) + convert_to_string(k) + convert_to_string(i);
						vv.add(IloNumVar(env, 0, 1, ILOBOOL, name.c_str()));
					}
					v.push_back(vv);
				}
		...
		for (unsigned int s = 1; s <= hps.size(); s++) {
			for (unsigned int j = 1; j <= hps.size(); j++) {
				...
				for (unsigned int k = 1; k <= K; k++) {
					...
					model.add(IloRange(env, 0, nf[j - 1][k - 1] - Delta - nsjk[s - 1][j - 1][k - 1], 0));
				}
			}
		}
		...
		// ***************** REFINED ANALYSIS ****************************** //
		// (4.4.1) check that sjk is not a valid finish time candidate
		...
		for (unsigned int k = 1; k <= K; k++) {
			for (unsigned int s = 1; s <= hps.size(); s++) {
				...
				model.add(IloRange(env, 0, totI + totIdle - Sjk - sigma1, IloInfinity));
			}
		}
